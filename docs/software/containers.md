...
Lastly, when using containers, all software and exact versions (image tags) are written in-line with your code, so you don't have to provide separate installation instructions or a list of specific dependencies that must first be available in order for your script to run anywhere else - it will all be contained in one file!
...
### Pre-built container images from biocontainers.pro (recommended)
Usually it's not necessary to build a container image yourself unless you want to customize things in detail, since there are a plethora of free, pre-built container images already publically available that work straight of the box. For bioinformatic software the community-driven project [biocontainers.pro](https://biocontainers.pro/) should have anything you need, and if not - you can contribute! All their container images are hosted on the container registry [quay.io](https://quay.io). Browse the list of available container images [here](https://quay.io/organization/biocontainers). The list is quite long though, so if you already know the name of the tool you need, it's quicker to just type it directly in the URL instead: `https://quay.io/repository/biocontainers/<toolname>`. For example `minimap2` is available here: [https://quay.io/repository/biocontainers/minimap2](https://quay.io/repository/biocontainers/minimap2).

Container images are tagged, usually one for each version or release, so you have to **pick a specific tag, otherwise you will likely get an error** - but that's perfect because it enforces reproducibility. Using specific tags ensures that the container image never (EVER) changes, and thus will continue to work even decades from now. **NEVER use a "`latest`" tag** if it's available, because you will get different versions depending on when the container image is pulled every time you run your code (though individual image tags are of course cached locally). This can cause downstream trouble, inconsistent output, and code that used to work just fine can suddenly break - all degrading reproducibility significantly. To obtain a tag name for a particular container image, pick a tag from the "Tags" menu to the left and copy it's name. For example to run `minimap2` you would likely choose the most recent tag `2.31--h118bc1c_0`:
...
Apptainer will cache images for you, which is best, simply because people often use some of the same tools across multiple projects. Alternatively, you can also pull the container images to a single file if necessary and run from that instead:

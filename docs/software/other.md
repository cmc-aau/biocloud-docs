...
## CLC
CLC version 25 is currently only installed on the `bio-node11` node in the `interactive` partition (currently also the only node there). You should be able to find CLC in the menus when starting a [virtual desktop](../guides/webportal/apps/virtualdesktop.md). As of November 2025, newer versions of CLC are licensed on a per-user basis and no longer tied to specific machines.

...
## CLC
CLC version 25 is currently only installed on (and licensed to) the `bio-node11` node in the `interactive` partition (currently the only node there). You should be able to find CLC in the menus when starting a [virtual desktop](../guides/webportal/apps/virtualdesktop.md).

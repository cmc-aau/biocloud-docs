...
### MPI jobs
MPI enables processes to communicate across nodes to provide limitless scalability for large jobs. It is, however, a requirement that the software used is specifically developed and compiled with MPI support. For running distributed multi-node [MPI jobs](https://slurm.schedmd.com/archive/slurm-24.11.4/mpi_guide.html) through slurm, see `srun --mpi=list` for available plugins, and then simply ensure that fx `srun --mpi=pmix` is used to submit tasks from the batch job. It is **not** recommended to use the `mpirun` or `mpiexec` commands when using the OpenMPI plugin as `srun` is configured to handle things automatically, see [details here](https://docs.open-mpi.org/en/main/launching-apps/slurm.html#using-slurm-s-direct-launch-functionality). Depending on the software used, or when using software through [containers](../software/containers.md), it may be necessary to test out different combinations of the `srun` and `mpirun` commands, as well as different plugins, on some small test data before scaling things up.
...
| `--mail-user`                                | Local user | A comma-separated list of email addresses where job notifications are sent. |

...
When a process is killed using `kill -9` (`SIGKILL` code), CephFS will not have the chance to properly close any open file handles, which can leave the Ceph MDS server with stale entries in its metadata cache. Whenever a client accesses a file, the Ceph MDS issues so called [capabilities](https://docs.ceph.com/en/quincy/cephfs/capabilities/) for the file (actually an inode, to be technically correct). The MDS then waits for the client to release back these capabilities when it's finished with the file, to signal that other clients can now access the file. If a process is killed too hard, the client may never release the capability again, which causes serious trouble on the MDS, and the only solution is rebooting the client or forcing an unmount, which will impact other jobs running on the node. If this happens too often, the MDS server will eventually run out of memory and start evicting clients as described above, but it can also lead to dead-locked folders, that are unaccessible until nodes, often multiple nodes if the file has been accessed from multiple places at once, are rebooted (see [this](https://docs.clyso.com/docs/kb/cephfs/#mds---mds_client_recall-mds_slow_request-mds_client_oldest_tid-warnings-lasting-for-many-hours)). Therefore, please always try to use `kill` (SIGTERM) first, which will allow the Ceph IO processes to catch the signal and close any open file handles properly before exiting.
